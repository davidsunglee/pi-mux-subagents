...
import { createSubagentActivityRecorder } from "../launch/activity.ts";
...
/**
 * Write a durable auto-exit `done` sidecar so the parent orchestrator can
 * observe completion even if the mux surface goes away before the next
 * poll iteration. Never overwrites an existing `.exit` file — a prior
 * `caller_ping` payload must win, since it carries the ping message that
 * the parent needs to surface to the user.
 */
export function writeAutoExitDoneSidecar(sessionFile: string): void {
  try {
    writeFileSync(`${sessionFile}.exit`, JSON.stringify({ type: "done" }), { flag: "wx" });
  } catch (err: any) {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("agent_end", (event, ctx) => {
    ...
    if (shouldExit) {
      ...
      if (sessionFile) writeAutoExitDoneSidecar(sessionFile);
      ctx.shutdown();
      return;
    }
